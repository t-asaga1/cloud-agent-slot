...
## AI 素材生成(fal.ai)のルール(2026-07-16 ユーザー指示)
予告などの演出ムービーを fal.ai で生成する際は、以下のワークフローを必ず守ること
(「1 AGENT 1 やり取り」ルールと組み合わせると、原則 プロンプト提示 → 生成 → 組込み は別々のランになる):

1. **動画を生成する前に、使用予定のプロンプトを最初にユーザーへ提示して承認を得ること**。
   ユーザーが方針を修正する場合があるため、承認前に生成へ進まない。
2. **動画を生成したら、組込み(取り込み・差し替え)まで進まず、まずユーザーに生成動画を確認してもらうこと**。
   生成物はランのアーティファクト(`/opt/cursor/artifacts/`)として提示する。
3. **生成した動画をユーザーの指示なしに勝手に再生成しないこと**(API 課金が余分に掛かるため)。
   ユーザーが動画を確認して出す指示に従う。
- API キーは Cloud Agent Secret **`FAL_KEY2`** を使用する(2026-07-16 に FAL_KEY から変更。生成スクリプトは `fal.config({ credentials: process.env.FAL_KEY2 })` で明示設定する)。
