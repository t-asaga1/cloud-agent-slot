...
add_rect(s, Inches(0.35), Inches(2.9), Inches(6.15), Inches(4.05), C_PANEL)
add_text(s, Inches(0.6), Inches(3.02), Inches(5.7), Inches(3.8), bullets([
    ("現行の版権機開発フロー(調査)", {"head": True, "bold": True, "size": 15, "color": C_GOLD}),
    ("企画 → 絵コンテ → 素材制作 → コンポジット(AfterEffects)→ オーサリング(実機組込み)", {"size": 12.5}),
    ("版元からの素材支給 or 版元指定のアニメ制作会社へ新規作画を発注(スケジュール・品質のばらつきが課題)", {"size": 12.5}),
    ("版権キャラを崩さないことが最重要(セルルック 3DCG 化などの技法)", {"size": 12.5}),
    ("全カットが版元監修の往復対象 → 期間長期化の主要因", {"size": 12.5}),
], size=12.5))
x2 = Inches(6.8)
add_rect(s, x2, Inches(2.9), Inches(6.15), Inches(4.05), C_PANEL)
add_text(s, x2 + Inches(0.25), Inches(3.02), Inches(5.7), Inches(3.8), bullets([
    ("AI による短縮ポイント(構想)", {"head": True, "bold": True, "size": 15, "color": C_GOLD}),
    ("支給アニメ素材を参照画像・参照動画として GPT Image 2 / Seedance へ連鎖 → 作風を維持した新規カットを自動生成", {"size": 12.5}),
    ("原作の名場面・キャラ相関から、予告体系・振分けテーブルなど演出仕様自体をエージェントが自動起案", {"size": 12.5}),
    ("設定資料との一致検品(顔・衣装・色)を自動化し、監修往復を削減", {"size": 12.5}),
    ("仕様書(Excel)→ SPEC 転記 → 実装 → 網羅テストの今回の流れをテンプレ化し、機種横断で再利用", {"size": 12.5}),
    ("※ 版権素材の権利処理と最終監修は人間の承認が必須", {"size": 12, "color": C_MUTE, "plain": True}),
], size=12.5))
...
add_rect(s, Inches(0.35), Inches(1.2), Inches(6.15), Inches(5.75), C_PANEL)
add_text(s, Inches(0.6), Inches(1.32), Inches(5.7), Inches(5.5), bullets([
    ("実機の制約(規則・調査結果)", {"head": True, "bold": True, "size": 15, "color": C_GOLD}),
    ("メイン基板(出玉・リール制御): ROM 16KB(制御領域 4.5KB + データ領域 3KB)・RAM 1KB(使用 512B)", {"size": 12.5}),
    ("容量制約からメインプログラムは実質アセンブラで開発される", {"size": 12.5}),
    ("5.5 号機以降は AT 抽せん等の出玉関連処理も全てメイン基板側", {"size": 12.5}),
    ("サブ基板(演出・液晶): 容量制限が緩く C/C++ 等で開発。サブメイン(演出抽せん)+ サブサブ(液晶描画)の 2 層構成", {"size": 12.5}),
    ("型式試験: 指定試験機関(保通協 / GLI Japan)の試験 → 公安委員会の検定が必要", {"size": 12.5}),
], size=12.5))
x2 = Inches(6.8)
add_rect(s, x2, Inches(1.2), Inches(6.15), Inches(5.75), C_PANEL)
add_text(s, x2 + Inches(0.25), Inches(1.32), Inches(5.7), Inches(5.5), bullets([
    ("変換アプローチ(構想)", {"head": True, "bold": True, "size": 15, "color": C_GOLD}),
    ("① コアロジック(役抽せん・リール制御・出玉)を TypeScript → C 言語へ移植。既存の網羅テスト・100 万 G シミュレーションを同値性検証ベクタとして流用", {"size": 12.5}),
    ("② メイン基板相当部は容量削減の上でアセンブラ化を検討(抽せんテーブルの分母付き整数設計はそのまま活かせる)", {"size": 12.5}),
    ("③ 自動変換ツール(ts2c 等)は ES3 の約 70% 対応・16bit 整数のみで実用性が限定的 → AI エージェントによる移植 + テスト同値性検証が現実的", {"size": 12.5}),
    ("④ 演出層はデータ駆動設計(direction.ts の対応表)のため、サブ基板向け演出データテーブルへ変換しやすい", {"size": 12.5}),
    ("⑤ 液晶素材の実機フォーマット変換 → 評価ボードでの動作確認へ段階的に進める", {"size": 12.5}),
], size=12.5))
