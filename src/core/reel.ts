 ...
 * - DDT(左リールバー狙い。SPEC「3.」確定事項): 到達保証・目押し保証・期待ランクが
 *   同値の候補間で「バー(黒・白)を窓内のできるだけ下段寄りに止める」選好(ddtScore)を追加。
 *   指定位置は 2 箇所: 黒バー(コマ 19)と白バー(コマ 9。2026-07-16 ユーザー指示で追加)。
 *   2 つのバーは 10 コマ離れているため狙い範囲は重ならず、窓内に同時表示もされない。
 *   ハズレ・取りこぼし時はバー狙いでバーが下段付近に停止し、スイカ成立時はスイカ
 *   (コマ 15・20 = 黒バー周辺 / コマ 5・10 = 白バー周辺)を引き込んで揃うため察知できる。
 *   チェリー成立時は、チェリーを窓内へ引き込める押下位置ならチェリー停止で察知できる。
 *   ※ 黒バー狙い(押下位置 14〜19)では左チェリー(コマ 13)が物理的に窓内へ
 *   届かないため取りこぼし(= 黒バー停止のまま)になる(最大 4 コマスベリの制約)。
 *   白バー狙い(押下位置 5〜9)はチェリー(コマ 8・13)が両側にあるため全域で察知できる。
...
/** DDT の目安図柄(左リール各 1 個: 黒バー = コマ 19 / 白バー = コマ 9) */
const DDT_TARGET_SYMBOLS: readonly ReelSymbol[] = ['BAR_BLACK', 'BAR_WHITE'];

/**
 * DDT(左リールバー狙い。SPEC「3.」確定事項)の停止位置選好スコア(小さいほど良い)。
 * 左リールの停止候補について「バー(黒・白)を窓内のできるだけ下段寄りに表示する」を表す:
 * 下段 = 0 / 中段 = 1 / 上段 = 2 / 窓外 = 3。
 * 指定位置は黒バー(コマ 19)と白バー(コマ 9。2026-07-16 ユーザー指示で追加)の 2 箇所。
 * バーは左リール各 1 個で互いに 10 コマ離れているため、バー狙い(押下位置が各バーの
 * 手前付近)のときだけ候補間で差がつき、それ以外の押下位置では全候補が同値
 * (= 選好は働かない)。上位の評価(到達保証・目押し保証・期待ランク)が同値の
 * 候補間でのみ働くため、引き込み・蹴飛ばしの正しさには影響しない
 * (網羅テストの固定値で回帰検証)。
 */
function ddtScore(reel: ReelIndex, position: number): number {
  if (reel !== 0) return 0;
  if (DDT_TARGET_SYMBOLS.includes(komaAt(0, position - 1))) return 0; // 下段
  if (DDT_TARGET_SYMBOLS.includes(komaAt(0, position))) return 1; // 中段
  if (DDT_TARGET_SYMBOLS.includes(komaAt(0, position + 1))) return 2; // 上段
  return 3;
}
 ...
 * 4. DDT 選好(ddtScore): 左リールのバー(黒・白)をできるだけ下段寄りに表示
 * 5. スベリコマ数最小
 */
