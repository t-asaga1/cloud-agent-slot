...
describe('停止制御 II(STEP 1d: レア役 × 全 20^3 押下位置 × 押し順 6 通りの網羅検証)', () => {
  ...
  it('代表例: チェリーの停止段の作り分け(左押下位置 3)と payout 配線', () => {
    ...
    expect(calcPayout(corner.displayed, true, corner.bellSuccess).payout).toBe(2);
    ...
    expect(calcPayout(center.displayed, true, center.bellSuccess).payout).toBe(2);
  ...
  it('代表例: チャンス目のテンパイはずし目と payout 配線(順押し [0, 0, 3])', () => {
    ...
    expect(calcPayout(chance.displayed, true, chance.bellSuccess).payout).toBe(3);
...
describe('停止制御 III(STEP 1e: リーチ目 × 全 20^3 押下位置 × 押し順 6 通りの網羅検証)', () => {
  ...
  it('代表例: 目押し成功で 7 揃い 3 枚、外すと取りこぼし 0 枚(payout 配線)', () => {
    ...
    expect(calcPayout(win.displayed, true, win.bellSuccess).payout).toBe(3);
