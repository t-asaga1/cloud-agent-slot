...
describe('リール配列(docs/SPEC.md 叩き台との整合)', () => {
  it('3 リール × 20 コマである', () => {
  ...
  it('スイカ・チェリーは取りこぼしが発生し得る配置(引き込めない押下位置がある)', () => {
    expect(ALL_POSITIONS.some((p) => !canReach(0, p, 'WATERMELON'))).toBe(true);
    expect(ALL_POSITIONS.some((p) => !canReachWeakCherry(p) && !canReachStrongCherry(p))).toBe(
      true,
    );
  });

  it('チェリーは左リールの弱(角)・強(中段)の両方の停止形を作れる', () => {
    expect(ALL_POSITIONS.some((p) => canReachWeakCherry(p))).toBe(true);
    expect(ALL_POSITIONS.some((p) => canReachStrongCherry(p))).toBe(true);
  });
});
...
describe('表示窓と出目判定', () => {
  ...
  it('中段 3 つ揃いのライン役を判定する', () => {
    ...
    // 左0=赤7, 中3=赤7, 右6=赤7
    expect(judgeDisplay([0, 3, 6])).toBe('BONUS_BIG');
    // 左7=BAR, 中7=BAR, 右10=BAR
    expect(judgeDisplay([7, 7, 10])).toBe('BONUS_REG');
  });

  it('左リールのチェリーを弱(角)・強(中段)で判定する', () => {
    // 左4: 下段=コマ3(チェリー)= 角チェリー(中段はスイカ、他リールはライン不成立の位置)
    expect(judgeDisplay([4, 0, 1] as StopPositions)).toBe('CHERRY_WEAK');
    // 左3: 中段=コマ3(チェリー)= 強チェリー
    expect(judgeDisplay([3, 0, 0] as StopPositions)).toBe('CHERRY_STRONG');
  });
});
...
function expectedDisplay(wonRole: Role, pushes: readonly [number, number, number]): Role {
  ...
  if (wonRole === 'WATERMELON' || wonRole === 'BONUS_BIG' || wonRole === 'BONUS_REG') {
  ...
  if (wonRole === 'CHERRY_WEAK') return canReachWeakCherry(pushes[0]) ? wonRole : 'NONE';
  if (wonRole === 'CHERRY_STRONG') return canReachStrongCherry(pushes[0]) ? wonRole : 'NONE';
...
describe('停止制御(全役 × 全 20^3 押下位置 × 全押し順の網羅検証)', () => {
  ...
  it('チェリー非当選時は左リールの窓内にチェリーが表示されない(ベル・リプレイの引き込み時を除く)', () => {
    const nonCherryRoles = ROLES.filter((r) => r !== 'CHERRY_WEAK' && r !== 'CHERRY_STRONG');
